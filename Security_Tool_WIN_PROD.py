...
#   1.12        Shaun Craig   10/02/2018    Finished Program For Dissertations       s.craig@2014.ljmu.ac.uk
#
    ...
    if not AttackTime:
        ...
        for WeakestHost in ([Time[0] for Time in AttackTime]):
            if AttackTime[counter][0] == ST:
                ...
                break
            counter = counter + 1
        #print("Weakest IP Address Target is ", WeakestIPAddress)
        ...
        print("The Weakest IP Address is", WeakestIPAddress, "with a ping of", ping, "which was attacked in", ST,
              "Seconds", "The password position of this was ", PasswordPlace, "th in password list")
     # ********************************************************************* Calculation
        FastestTime = ST
        #   ***************************** NETWORK SCORE
...
print("Scanning IP Range" , network_submask + "...                   (This May Take Some Time To Start)")
...
# ************************************ Uses a worker per port *********
        q = Queue()
        for x in range (20): # how
            # many processes it runs at the same time LOWER FOR LOWER CPUs
...
if vulnerabilities_counter > 0:
    ...
    print("-------------------------------------------------------------------------------------------------------------------")
    print("Dictionary attacking to provide password strength statistics requires Ethical Approval")
    print("Ethical Approval can only come from the person who owns the network and must be 16 or over")
